...
const MarketList = (props) => {
    ...
    fetch(
      ...
      .then((result) => {
        if (result.success == true) {
          ...
          setSelectedOdd([...selectedOdd, ...result.sys_odd]);
          setSelectedEven([...selectedEven, ...result.sys_even]);
      ...
      fetch(
        ...
        .then((result) => {
          if (result.success == true) {
            ...
            setSelectedOdd(
              selectedOdd.filter((item) => !result.data.includes(item))
            );

            setSelectedEven(
              selectedEven.filter((item) => !result.data.includes(item))
            );
  ...
  const addRunner = (e, data, tr, odd_even) => {
    setLoading(true);
    if (e.target.checked == true) {
      ...
      fetch(
        import.meta.env.VITE_API_HOST + "/event/selectOddEven",
        requestOptions
      )
        .then((response) => {
          if (response.status === 403) {
  ...
  return (
    <>
      <div className="row" key="markets">
        ...
        {marketList.map((v, k) => (
          <>
            <div className="col-12 col-md-6 p-0" key={k}>
                              ...
                              <ul className="list-group">
                                ...
                                >
                                  <table className="table">
                                    <tr>
                                      <th className="text-success">ODD </th>

                                      <th className="text-success">Player</th>
                                      <th className="text-success">EVEN </th>
                                    </tr>
                                    <tbody>
                                      {playerList.map((v1, i) =>
                                              ...
                                              <td className="even">
                                                ...
                                                v.parent_locked == 0 ? (
                                                  <input
                                                    ...
                                                    disabled={selectedEven.includes(
                                                      v1.id
                                                    )}
                                                  />
                                                ) : (
                        ...
                        <div className="row" key={k}>
                                ...
                                v1.market_id == v.id && v1.type == "user" ? (
                                  <>
                                    <li className="list-group-item list-group-item-info d-flex justify-content-between align-items-center">
                                      ...
                                      <i className="badge badge-success badge-pill">{v1.runner_name}</i>
                                      -
                                      <i className="uppercase">{v1.odd_even}</i><CheckIcon className="w-6 text-success"></CheckIcon>
                                    </li>
                                  </>
                                ) : null
                              )}
                          ...
                          <div className="col">
                            <ul className="list-group">
                              ...
                              {confirmPlayers.map((v1, i) =>
                                v1.market_id == v.id && v1.type == "system" ? (
                                  <>
                                    <li className="list-group-item list-group-item-secondary d-flex justify-content-between align-items-center">
                                      {/* <input
                                        ...
                                        className="text-success bg-success"
                                      /> */}
                                     <CheckCircleIcon className="w-6 text-secondary"></CheckCircleIcon>
                                      
                                     <i className="badge badge-secondary badge-pill">{v1.runner_name}</i>
                                      -
                                      <i className="uppercase">{v1.odd_even}</i><CheckIcon className="w-6 text-secondary"></CheckIcon>
                                    </li>
                                  </>
                                ) : null
                              )}
                          ...
                          <div className="col-12 mt-1">
                            <i className="text-success">Players Confirmed</i>
                            <span>
                              <CheckCircleIcon className="ml-1 w-6 text-success"></CheckCircleIcon>{" "}
                              VS{" "}
                              <CheckCircleIcon className="ml-1 w-6 text-secondary"></CheckCircleIcon>
                            </span>
