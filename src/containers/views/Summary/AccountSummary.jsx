...
const AccountSummary = (props) => {
    ...
    const handleSubmit = (u_id = '') => {
        ...
        fetch(import.meta.env.VITE_API_HOST + "/account/getacsummary", requestOptions)
            .then((response) => {
    ...
    if (data_ob.length > 0) {
        ac_data = data_ob.map((v, k) => {
            ...
            total_upline += parseFloat(v.up_line * (-1));
            return (
                <React.Fragment key={k}>
                    <tr className={`text-black dark:text-white ${(k) % 2 === 0 ? "bg-white" : "bg-light"}`} >
                        ...
                        <td className={`px-2 py-0.5 text-right ${(v.amount) < 0 ? "text-danger" : "text-success"}`}>
                            {v.amount}
                        </td>
                        {props.role < 5 && <td className={`px-2 py-0.5 text-right ${(v.up_line) > 0 ? "text-danger" : "text-success"}`}>
                            {v.up_line * (-1)}
                        </td>}
                    </tr>
                </React.Fragment>
